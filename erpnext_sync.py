...
def main():
    ...
    try:
        ...
        if (last_lift_off_timestamp and last_lift_off_timestamp < datetime.datetime.now() - datetime.timedelta(minutes=int(config.get('PULL_FREQUENCY')))) or not last_lift_off_timestamp:
...
def pull_process_and_push_data(device, device_attendance_logs=None):
    ...
    attendance_success_logger = setup_logger(attendance_success_log_file, '/'.join([config.get('LOGS_DIRECTORY'), attendance_success_log_file])+'.log')
    attendance_failed_logger = setup_logger(attendance_failed_log_file, '/'.join([config.get('LOGS_DIRECTORY'), attendance_failed_log_file])+'.log')
    if not device_attendance_logs:
        device_attendance_logs = get_all_attendance_from_device(device['ip'], device_id=device['device_id'],password= device['password'], clear_from_device_on_fetch=device['clear_from_device_on_fetch'])
        if not device_attendance_logs:
            return
    ...
    index_of_last = -1
    last_line = get_last_line_from_file('/'.join([config.get('LOGS_DIRECTORY'), attendance_success_log_file])+'.log')
    import_start_date = _safe_convert_date(config.get('IMPORT_START_DATE'), "%Y%m%d")
    ...
    for device_attendance_log in device_attendance_logs[index_of_last+1:]:
        ...
        erpnext_status_code, erpnext_message = send_to_erpnext(device_attendance_log['user_id'], device_attendance_log['timestamp'],device_attendance_log['punch'],device['device_id'])
...
def get_all_attendance_from_device(ip, port=4370, timeout=30, password=None,device_id=None, clear_from_device_on_fetch=False):
...
def send_to_erpnext(employee_field_value, timestamp, log_type=None, device_id=0):
    
    """
    Example: send_to_erpnext('12349',datetime.datetime.now(),'HO1','IN')
    """
    url = config.get('ERPNEXT_URL') + "/api/method/newline_hr_v13.api.attendance_and_departure_movements"
    headers = {
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if EMPLOYEE_NOT_FOUND_ERROR_MESSAGE in error_str:
                error_logger.error('\t'.join(['Error during ERPNext API Call.', str(employee_field_value), str(timestamp.timestamp()), str(log_type), error_str]))
                # TODO: send email?
            else:
                error_logger.error('\t'.join(['Error during ERPNext API Call.', str(employee_field_value), str(timestamp.timestamp()), str(log_type), error_str]))
...
def send_shift_sync_to_erpnext(shift_type_name, sync_timestamp):
    url = config.get('ERPNEXT_URL') + "/api/resource/Shift Type/" + shift_type_name
    headers = {
        'Authorization': "token "+ config.get('ERPNEXT_API_KEY') + ":" + config.get('ERPNEXT_API_SECRET'),
        'Accept': 'application/json'
    }
...
def get_dump_file_name_and_directory(device_id, device_ip):
    return config.get('LOGS_DIRECTORY') + '/' + device_id + "_" + device_ip.replace('.', '_') + '_last_fetch_dump.json'
...
if not os.path.exists(config.get('LOGS_DIRECTORY')):
    os.makedirs(config.get('LOGS_DIRECTORY'))
error_logger = setup_logger('error_logger', '/'.join([config.get('LOGS_DIRECTORY'), 'error.log']), logging.ERROR)
info_logger = setup_logger('info_logger', '/'.join([config.get('LOGS_DIRECTORY'), 'logs.log']))
status = pickledb.load('/'.join([config.get('LOGS_DIRECTORY'), 'status.json']), True)
...
class SyncService(Thread):
    stop = False
    def __init__(self,queue,sleep_time=15):
